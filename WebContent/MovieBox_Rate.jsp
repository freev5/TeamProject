...
#rateB:hover{color:#5fda5f;}
	...
	while(true){
		...
		for(int ii=0;ii<md.size();ii++){
			for(int jj=0;jj<wish.size();jj++){
				if(rate.get(i).getId().equals(wish.get(jj).getId())&&wish.get(jj).getMtitle().equals(md.get(ii).getMtitle())){
					wishOn = 1;
					break;
				}
				wishOn = 0;
			}
				if(id.equals(rate.get(i).getId())&&rate.get(i).getMtitle().equals(md.get(ii).getMtitle())){
					initialRate=Integer.parseInt(rate.get(i).getRating());
					%>
